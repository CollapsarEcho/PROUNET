...
parser = argparse.ArgumentParser(
    description="RADIOLOGY-2019 Prostate Lesion Segmentation training script"
)
...
parser.add_argument(
    "--workers", default=8, type=int, metavar="N", help="number of data loading workers"
)
parser.add_argument(
...
parser.add_argument(
    "--num_splits", default=10, type=int, help="split dataset in 10 folds"
)
parser.add_argument(
    "--num_val_folds", default=2, type=int, help="leave x fold out for validation"
)
parser.add_argument(
    "--num_test_folds", default=2, type=int, help="leave x fold out for testing"
)
parser.add_argument("--seed", default=42, type=int, help="seed")
parser.add_argument(
    "--patch_size", default=(160, 160), type=tuple, help="define patch size"
)
parser.add_argument(
    "--p", default=1, type=float, help="probability for spatial transform"
)
parser.add_argument(
    "--cv_number", default=16, type=int, help="number of cross validation loops"
)
...
def main():
    ...
    for cv in range(args.cv_start, args.cv_end):
        ...
        print(
            "T2", Tumor_frequencie_T2, Prostate_frequencie_T2, Background_frequencie_T2
        )

        all_ADC = np.float(
            Background_frequencie_ADC + Prostate_frequencie_ADC + Tumor_frequencie_ADC
        )
        all_T2 = np.float(
            Background_frequencie_T2 + Prostate_frequencie_T2 + Tumor_frequencie_T2
        )
        ...
        optimizer = torch.optim.Adam(
            model.parameters(), lr, weight_decay=args.weight_decay
        )
