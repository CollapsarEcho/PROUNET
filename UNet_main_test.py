...
parser = argparse.ArgumentParser(
    description="RADIOLOGY-2019 Prostate Lesion Segmentation ensemble testing script"
)

parser.add_argument("--input_file", default="", type=str, help="name of input file")
parser.add_argument(
    "--ensemble_path", default="", type=str, help="path to pretrained CNN ensemble"
)
parser.add_argument(
    "--checkpoint_name",
    default="checkpoint_UNetPytorch.pth.tar",
    type=str,
    help="name of ensemble checkpoint files",
)
...
def main():
    ...
    print(
        "ADC", Tumor_frequencie_ADC, Prostate_frequencie_ADC, Background_frequencie_ADC
    )
    ...
    for folder, subfolders, files in sorted(os.walk(args.ensemble_path)):
        for file in files:
            if file.startswith(args.checkpoint_name):
                ...
                folder_name_for_single_nets = general_folder_name + "/Net_{}".format(
                    count
                )
                ...
                test_loss = validate(
                    test_loader,
                    model,
                    folder_name=folder_name_for_single_nets,
